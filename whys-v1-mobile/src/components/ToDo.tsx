...
import { toggleActionItem } from "../api/actionitems";
...
export default function ActionItemsSection({ data: overrideData, style }: Props) {
  ...
  // ✅ FIXED: Prevent duplicate "All" by excluding "all" from database
  const categories = useMemo(() => {
    const set = new Set<string>();
    localItems.forEach((i) => {
      // Skip the database default "all" category
      if (i.category.toLowerCase() === 'all') return;
    ...
    // Always add "All" as the first filter option
    return ["All", ...Array.from(set).sort()];
  }, [localItems]);

  // ✅ FIXED: Filter treats "All" as show everything, ignores database "all" category
  const filteredItems =
      ...
      >
        ...
        {error && (
          <View style={s.errorContainer}>
            <Ionicons name="alert-circle-outline" size={v(26)} color="#b00020" />
